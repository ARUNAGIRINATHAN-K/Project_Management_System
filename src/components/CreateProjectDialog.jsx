...
const CreateProjectDialog = ({ isDialogOpen, setIsDialogOpen }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            // Find lead member user object
            const leadMember = currentWorkspace?.members?.find(m => m.user.email === formData.team_lead);
            const teamLeadId = leadMember ? leadMember.user.id : "user_1";
            
            // Map selected emails back to member structures
            const projectMembers = formData.team_members.map(email => {
                ...
                return {
                    ...
                    userId: workspaceMember ? workspaceMember.user.id : "user_1",
                    user: workspaceMember ? workspaceMember.user : { id: "user_1", name: email, email }
                };
            });

            // Ensure the team lead is also in the members array if not already present
            if (leadMember && !projectMembers.some(m => m.userId === leadMember.user.id)) {
                projectMembers.push({
                    id: `pm_${Math.random().toString(36).substr(2, 9)}`,
                    userId: leadMember.user.id,
