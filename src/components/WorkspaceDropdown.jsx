...
function WorkspaceDropdown() {
    ...
    const getWorkspaceImage = (ws) => {
        return ws?.image_url || `https://api.dicebear.com/7.x/initials/svg?seed=${encodeURIComponent(ws?.name || "Workspace")}`;
    };
        ...
        <div className="relative m-4" ref={dropdownRef}>
            <button onClick={() => setIsOpen(prev => !prev)} className="w-full flex items-center justify-between p-3 h-auto text-left rounded hover:bg-gray-100 dark:hover:bg-zinc-800" >
                <div className="flex items-center gap-3">
                    <img src={getWorkspaceImage(currentWorkspace)} alt={currentWorkspace?.name} className="w-8 h-8 rounded shadow" />
                    ...
                    <div className="p-2">
                        ...
                        {workspaces.map((ws) => (
                            <div key={ws.id} onClick={() => onSelectWorkspace(ws.id)} className="flex items-center gap-3 p-2 cursor-pointer rounded hover:bg-gray-100 dark:hover:bg-zinc-800" >
                                <img src={getWorkspaceImage(ws)} alt={ws.name} className="w-6 h-6 rounded" />
